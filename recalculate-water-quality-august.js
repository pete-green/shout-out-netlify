/**
 * Recalculate Water Quality for August 2025 from Database
 */
...
async function recalculateAugust() {
  console.log('🚀 Recalculating Water Quality for August 2025');
  console.log('='.repeat(80));
  const startTime = Date.now();

  await loadCrossSaleGroupCache();

  console.log('📡 Fetching August estimates...');
  const { data: estimates, error } = await supabase
    .from('estimates')
    .select('id, estimate_id, salesperson, amount, sold_at, raw_data')
    .gte('sold_at', '2025-08-01T00:00:00Z')
    .lt('sold_at', '2025-09-01T00:00:00Z')
    .order('sold_at');

  if (error) throw error;
  console.log(`✅ Found ${estimates.length} August estimates\n`);
  ...
  console.log('\n' + '='.repeat(80));
  console.log('✅ AUGUST 2025 COMPLETE');
  console.log('='.repeat(80));
