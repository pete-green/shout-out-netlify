// Backfill script for May 2025 with PAGINATION support
// Run with: node backfill-may-2025.js
...
async function backfillMay() {
  try {
    console.log('🔄 ===== STARTING MAY 2025 BACKFILL (WITH PAGINATION) =====\n');
    ...
    // Fetch May estimates (May 1, 2025 at midnight)
    const startDate = '2025-05-01T00:00:00';
    const endDate = new Date('2025-06-01T00:00:00'); // Through end of May
    ...
    // Filter to May only (before July 1)
    const mayEstimates = allEstimates.filter((est) => {
      const soldDate = new Date(est.soldOn);
      return soldDate < endDate;
    });

    console.log(`📊 Filtered to ${mayEstimates.length} estimates in May\n`);

    // Check which ones exist in database
    const estimateIds = mayEstimates.map(e => e.id);
    ...
    for (let i = 0; i < mayEstimates.length; i++) {
      const estimate = mayEstimates[i];
      const estimateId = estimate.id;

      // Only fetch names if this is new or needs updating
      console.log(`Processing ${i + 1}/${mayEstimates.length}: ${estimateId}...`);
