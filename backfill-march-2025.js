// Backfill script for March 2025 with PAGINATION support
// Run with: node backfill-march-2025.js
...
// Main function
async function backfillMarch() {
  try {
    console.log('🔄 ===== STARTING MARCH 2025 BACKFILL (WITH PAGINATION) =====\n');
    ...
    // Fetch March estimates (March 1, 2025 at midnight)
    const startDate = '2025-03-01T00:00:00';
    const endDate = new Date('2025-04-01T00:00:00'); // Through end of March
    ...
    // Filter to March only (before July 1)
    const marchEstimates = allEstimates.filter((est) => {
      const soldDate = new Date(est.soldOn);
      return soldDate < endDate;
    });

    console.log(`📊 Filtered to ${marchEstimates.length} estimates in March\n`);

    // Check which ones exist in database
    const estimateIds = marchEstimates.map(e => e.id);
    ...
    for (let i = 0; i < marchEstimates.length; i++) {
      const estimate = marchEstimates[i];
      const estimateId = estimate.id;

      // Only fetch names if this is new or needs updating
      console.log(`Processing ${i + 1}/${marchEstimates.length}: ${estimateId}...`);
