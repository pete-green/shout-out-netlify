/**
 * Recalculate Water Quality for November 2025 from Database
 *
...
async function recalculateNovember() {
  console.log('🚀 Recalculating Water Quality for November 2025');
  ...
  // Fetch all November estimates from database
  console.log('📡 Fetching November estimates from database...');
  const { data: estimates, error } = await supabase
    .from('estimates')
    .select('id, estimate_id, salesperson, amount, sold_at, raw_data')
    .gte('sold_at', '2025-11-01T00:00:00Z')
    .lt('sold_at', '2025-12-01T00:00:00Z')
    .order('sold_at');
  ...
  console.log(`✅ Found ${estimates.length} November estimates\n`);
...
recalculateNovember().catch(error => {
  console.error('\n💥 ERROR:', error);
  process.exit(1);
});
