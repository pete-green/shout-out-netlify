// Backfill script for June 2025 with PAGINATION support
// Run with: node backfill-june-2025.js
...
async function backfillJune() {
  try {
    ...
    // Fetch June estimates (June 1, 2025 at midnight)
    const startDate = '2025-06-01T00:00:00';
    const endDate = new Date('2025-07-01T00:00:00'); // Through end of June
    ...
    // Filter to June only (before July 1)
    const juneEstimates = allEstimates.filter((est) => {
      const soldDate = new Date(est.soldOn);
      return soldDate < endDate;
    });

    console.log(`📊 Filtered to ${juneEstimates.length} estimates in June\n`);

    // Check which ones exist in database
    const estimateIds = juneEstimates.map(e => e.id);
    ...
    for (let i = 0; i < juneEstimates.length; i++) {
      const estimate = juneEstimates[i];
      const estimateId = estimate.id;

      // Only fetch names if this is new or needs updating
      console.log(`Processing ${i + 1}/${juneEstimates.length}: ${estimateId}...`);
    ...
    console.log(`   New Records: ${juneEstimates.length - existingIds.size}`);
    console.log('\n═══════════════════════════════════════════════════\n');
