// Backfill script for September 2025 with PAGINATION support
// Run with: node backfill-september-2025.js
...
async function backfillSeptember() {
  try {
    console.log('🔄 ===== STARTING SEPTEMBER 2025 BACKFILL (WITH PAGINATION) =====\n');
    ...
    // Fetch September estimates (Sept 1, 2025 at midnight)
    const startDate = '2025-09-01T00:00:00';
    const endDate = new Date('2025-10-01T00:00:00'); // Through end of September
    ...
    // Filter to September only (before Oct 1)
    const septemberEstimates = allEstimates.filter((est) => {
      const soldDate = new Date(est.soldOn);
      return soldDate < endDate;
    });

    console.log(`📊 Filtered to ${septemberEstimates.length} estimates in September\n`);

    // Check which ones exist in database
    const estimateIds = septemberEstimates.map(e => e.id);
    ...
    for (let i = 0; i < septemberEstimates.length; i++) {
      const estimate = septemberEstimates[i];
      const estimateId = estimate.id;

      // Only fetch names if this is new or needs updating
      console.log(`Processing ${i + 1}/${septemberEstimates.length}: ${estimateId}...`);
    ...
    console.log(`   New Records: ${septemberEstimates.length - existingIds.size}`);
    console.log('\n═══════════════════════════════════════════════════\n');
    ...
    console.log('\n✅ ===== SEPTEMBER 2025 BACKFILL COMPLETE =====');
