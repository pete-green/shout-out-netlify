// Backfill script for April 2025 with PAGINATION support
// Run with: node backfill-april-2025.js
...
// Main function
async function backfillApril() {
  try {
    console.log('🔄 ===== STARTING APRIL 2025 BACKFILL (WITH PAGINATION) =====\n');
    ...
    // Fetch April estimates (April 1, 2025 at midnight)
    const startDate = '2025-04-01T00:00:00';
    const endDate = new Date('2025-05-01T00:00:00'); // Through end of April
    ...
    // Filter to April only (before July 1)
    const aprilEstimates = allEstimates.filter((est) => {
      const soldDate = new Date(est.soldOn);
      return soldDate < endDate;
    });

    console.log(`📊 Filtered to ${aprilEstimates.length} estimates in April\n`);

    // Check which ones exist in database
    const estimateIds = aprilEstimates.map(e => e.id);
    ...
    for (let i = 0; i < aprilEstimates.length; i++) {
      const estimate = aprilEstimates[i];
      const estimateId = estimate.id;

      // Only fetch names if this is new or needs updating
      console.log(`Processing ${i + 1}/${aprilEstimates.length}: ${estimateId}...`);
