// Backfill script for August 2025 with PAGINATION support
// Run with: node backfill-august-2025.js
...
// Main function
async function backfillAugust() {
  try {
    console.log('🔄 ===== STARTING AUGUST 2025 BACKFILL (WITH PAGINATION) =====\n');
    ...
    // Fetch August estimates (Aug 1, 2025 at midnight)
    const startDate = '2025-08-01T00:00:00';
    const endDate = new Date('2025-09-01T00:00:00'); // Through end of August
    ...
    // Filter to August only (before Sept 1)
    const augustEstimates = allEstimates.filter((est) => {
      const soldDate = new Date(est.soldOn);
      return soldDate < endDate;
    });

    console.log(`📊 Filtered to ${augustEstimates.length} estimates in August\n`);

    // Check which ones exist in database
    const estimateIds = augustEstimates.map(e => e.id);
    ...
    for (let i = 0; i < augustEstimates.length; i++) {
      const estimate = augustEstimates[i];
      const estimateId = estimate.id;

      // Only fetch names if this is new or needs updating
      console.log(`Processing ${i + 1}/${augustEstimates.length}: ${estimateId}...`);
    ...
    console.log(`   New Records: ${augustEstimates.length - existingIds.size}`);
    console.log('\n═══════════════════════════════════════════════════\n');
