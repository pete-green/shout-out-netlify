// Backfill script for February 2025 with PAGINATION support
// Run with: node backfill-february-2025.js
...
// Main function
async function backfillFebruary() {
  try {
    console.log('🔄 ===== STARTING FEBRUARY 2025 BACKFILL (WITH PAGINATION) =====\n');
    ...
    // Fetch February estimates (February 1, 2025 at midnight)
    const startDate = '2025-02-01T00:00:00';
    const endDate = new Date('2025-03-01T00:00:00'); // Through end of February
    ...
    // Filter to February only (before July 1)
    const februaryEstimates = allEstimates.filter((est) => {
      const soldDate = new Date(est.soldOn);
      return soldDate < endDate;
    });

    console.log(`📊 Filtered to ${februaryEstimates.length} estimates in February\n`);

    // Check which ones exist in database
    const estimateIds = februaryEstimates.map(e => e.id);
    ...
    for (let i = 0; i < februaryEstimates.length; i++) {
      const estimate = februaryEstimates[i];
      const estimateId = estimate.id;

      // Only fetch names if this is new or needs updating
      console.log(`Processing ${i + 1}/${februaryEstimates.length}: ${estimateId}...`);
    ...
    console.log('\n═══════════════════════════════════════════════════\n');
    console.log('📋 SUMMARY:');
    console.log(`   Total Estimates: ${februaryEstimates.length}`);
    console.log(`   TGLs Found: ${tglCount}`);
    console.log(`   Big Sales Found: ${bigSaleCount}`);
    console.log(`   Already in DB: ${existingIds.size}`);
    console.log(`   New Records: ${februaryEstimates.length - existingIds.size}`);
    console.log('\n═══════════════════════════════════════════════════\n');
    ...
    console.log('\n✅ ===== FEBRUARY 2025 BACKFILL COMPLETE =====');
