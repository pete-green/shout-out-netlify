// Backfill script for January 2025 with PAGINATION support
// Run with: node backfill-january-2025.js
...
async function backfillJanuary() {
  try {
    console.log('🔄 ===== STARTING JANUARY 2025 BACKFILL (WITH PAGINATION) =====\n');
    ...
    // Fetch January estimates (January 1, 2025 at midnight)
    const startDate = '2025-01-01T00:00:00';
    const endDate = new Date('2025-02-01T00:00:00'); // Through end of January
    ...
    // Filter to January only (before July 1)
    const januaryEstimates = allEstimates.filter((est) => {
      const soldDate = new Date(est.soldOn);
      return soldDate < endDate;
    });

    console.log(`📊 Filtered to ${januaryEstimates.length} estimates in January\n`);

    // Check which ones exist in database
    const estimateIds = januaryEstimates.map(e => e.id);
    ...
    for (let i = 0; i < januaryEstimates.length; i++) {
      const estimate = januaryEstimates[i];
      const estimateId = estimate.id;

      // Only fetch names if this is new or needs updating
      console.log(`Processing ${i + 1}/${januaryEstimates.length}: ${estimateId}...`);
    ...
    console.log('\n═══════════════════════════════════════════════════\n');
    console.log('📋 SUMMARY:');
    console.log(`   Total Estimates: ${januaryEstimates.length}`);
    console.log(`   TGLs Found: ${tglCount}`);
    console.log(`   Big Sales Found: ${bigSaleCount}`);
    console.log(`   Already in DB: ${existingIds.size}`);
    console.log(`   New Records: ${januaryEstimates.length - existingIds.size}`);
    console.log('\n═══════════════════════════════════════════════════\n');
    ...
    console.log('\n✅ ===== JANUARY 2025 BACKFILL COMPLETE =====');
