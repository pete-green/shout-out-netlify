// Backfill script for July 2025 with PAGINATION support
// Run with: node backfill-july-2025.js
...
// Main function
async function backfillJuly() {
  try {
    console.log('🔄 ===== STARTING JULY 2025 BACKFILL (WITH PAGINATION) =====\n');
    ...
    // Fetch July estimates (July 1, 2025 at midnight)
    const startDate = '2025-07-01T00:00:00';
    const endDate = new Date('2025-08-01T00:00:00'); // Through end of July
    ...
    // Filter to July only (before Aug 1)
    const julyEstimates = allEstimates.filter((est) => {
      const soldDate = new Date(est.soldOn);
      return soldDate < endDate;
    });

    console.log(`📊 Filtered to ${julyEstimates.length} estimates in July\n`);

    // Check which ones exist in database
    const estimateIds = julyEstimates.map(e => e.id);
    ...
    for (let i = 0; i < julyEstimates.length; i++) {
      const estimate = julyEstimates[i];
      const estimateId = estimate.id;

      // Only fetch names if this is new or needs updating
      console.log(`Processing ${i + 1}/${julyEstimates.length}: ${estimateId}...`);
    ...
    console.log(`   New Records: ${julyEstimates.length - existingIds.size}`);
    console.log('\n═══════════════════════════════════════════════════\n');
